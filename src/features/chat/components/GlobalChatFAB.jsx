...
export default function GlobalChatFAB() {
  ...
  return (
    <>
      {/* Mobile Floating View: Hidden on >= lg */}
      <div className="lg:hidden block fixed z-50 pointer-events-none inset-0">
        ...
        <button
          ...
          className={`absolute bottom-6 right-6 p-4 rounded-full bg-brand-500 text-black shadow-lg shadow-brand-500/30 hover:scale-110 active:scale-95 transition-all duration-300 flex items-center justify-center pointer-events-auto ${
            isMobileOpen ? 'scale-0 opacity-0 pointer-events-none' : 'scale-100 opacity-100'
          }`}
        ...
        <AnimatePresence>
          {isMobileOpen && (
            ...
            >
              <ChatUI isFloating={true} onClose={() => setIsMobileOpen(false)} />
            </motion.div>
          )}
        </AnimatePresence>
