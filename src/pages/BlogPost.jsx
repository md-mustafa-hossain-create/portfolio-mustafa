  ...
  lines.forEach((line, idx) => {
    ...
    if (line.trim().startsWith('- ')) {
      renderedElements.push(
        <li
          key={idx}
          className="text-xs sm:text-sm text-zinc-400 ml-4 list-disc mb-1.5 leading-relaxed font-sans"
        >
    ...
    // Paragraph text
    renderedElements.push(
      <p key={idx} className="text-xs sm:text-sm text-zinc-400 leading-relaxed mb-4 font-sans">
...
export default function BlogPost() {
  ...
  useEffect(() => {
    if (loading || !blog) return; // Wait until content is fully loaded and mounted
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-zinc-950 text-zinc-100 py-32 flex flex-col items-center justify-center font-sans">
        ...
        <span className="text-zinc-400 font-semibold text-sm">Loading article...</span>
      </div>
    );
  }
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <article className="space-y-8 text-left">
          ...
          <div className="space-y-4">
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-white tracking-tight leading-tight pt-2">
              {blog.title}
            </h1>
